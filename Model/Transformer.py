...
class Transformer(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, input_pad_idx: int, output_pad_idx: int, d_model: int = 200, num_head: int = 8, num_e_layer: int = 6,
    ...
    def forward(self, src: torch.Tensor, trg: torch.Tensor, src_mask: torch.Tensor = None,
        ...
        embedded_src = self.enc_PE(self.enc_embed(src))
        embedded_trg = self.dec_PE(self.dec_embed(trg))
